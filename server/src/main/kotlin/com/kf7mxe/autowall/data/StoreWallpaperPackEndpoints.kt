...
object StoreWallpaperPackEndpoints : ServerBuilder() {

    val info = Server.database.modelInfo(
        auth = UserAuth.require(),
        permissions = {
            val admin: Condition<StoreWallpaperPack> = if (this.auth.userRole() >= UserRole.ADMIN) Condition.Always else Condition.Never
            val mine = condition<StoreWallpaperPack> {
                it.creatorId.eq(this.auth.id) }
            val own = condition<StoreWallpaperPack> { it.creatorId eq auth.id }
